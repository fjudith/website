...
(e.g. `workspaces/fjudith/`), set `working-directory` and `DOCUSAURUS_BASE_URL`:
...
defaults:
  run:
    working-directory: workspaces/fjudith
jobs:
  build:
      ...
      - uses: actions/setup-node@v4
        with:
          ...
          cache-dependency-path: |
            workspaces/fjudith/package-lock.json
            workspaces/fjudith/slides/package-lock.json
      - run: npm ci
      # Decks are a separate npm workspace with their own Slidev versions.
      - run: npm ci
        working-directory: workspaces/fjudith/slides
      - run: npm run build
      - uses: actions/upload-pages-artifact@v3
        with:
          path: workspaces/fjudith/build
  deploy:
    needs: build
    runs-on: ubuntu-latest
    environment:

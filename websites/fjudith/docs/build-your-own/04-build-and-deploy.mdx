...
(e.g. `websites/fjudith/`), set `working-directory` and `DOCUSAURUS_BASE_URL`:
...
defaults:
  run:
    working-directory: websites/fjudith
jobs:
  build:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 20
          cache: npm
          cache-dependency-path: websites/fjudith/package-lock.json
      - run: npm ci
      - run: npm run build
      - uses: actions/upload-pages-artifact@v3
        with:
          path: websites/fjudith/build
  deploy:
    needs: build
    runs-on: ubuntu-latest
    environment:
